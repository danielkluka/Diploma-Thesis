...
bool QuectelBC660::manualRegisterToNetwork(uint32_t operatorCode, uint8_t mode, uint8_t format, uint32_t timeout)
{
    ...
    wakeUp();
    sprintf(_buffer, "AT+COPS=%d,%d,%d", mode, format, operatorCode);
    Serial.print("Sending command: ");
    Serial.println(_buffer); // Debug: vypíše príkaz na Serial monitor

    if (sendAndWaitFor(_buffer, _IP, timeout))
    {   
        Serial.print("Response: ");
        Serial.println(_buffer); // Debug: vypíše odpoveď modemu
        return true;
    } else {
...
bool QuectelBC660::writePemTLS(const char* tag, const char* pem, uint32_t timeout)
{
    if (_debug) { Serial.print(F("[TLS] Uploading: ")); Serial.println(tag); }

    sprintf(_buffer, "AT+QSSLCFG=0,0,\"%s\"", tag);
    wakeUp();
    if (!sendAndCheckReply(_buffer, ">", 2000)) return false;

    const char* p = pem;
    while (*p) {
        const char* e = strchr(p, '\n');
        if (!e) e = p + strlen(p);
        const char* lineEnd = (e > p && *(e - 1) == '\r') ? e - 1 : e;
        _uart->write((const uint8_t*)p, lineEnd - p);
        _uart->write("\r\n");
        p = (*e) ? e + 1 : e;
    }
    _uart->flush();
    ...
    if (!readReply(timeout, 2)) return false;
    return strstr(_buffer, "+QSSLCFG") != nullptr;
}

bool QuectelBC660::configTLS(const char* ca, const char* cli, const char* key, uint8_t lvl)
{
    sprintf(_buffer, "AT+QSSLCFG=0,0,\"seclevel\",%d", lvl);
    if (!sendAndCheckReply(_buffer, _OK)) return false;
    if (ca && !writePemTLS("cacert", ca, 300000)) return false;
    if (cli && !writePemTLS("clientcert", cli, 300000)) return false;
    if (key && !writePemTLS("clientkey", key, 300000)) return false;
    return true;
}


// MQTT functions
bool QuectelBC660::openMQTTSecure(const char* host, uint16_t port, uint8_t TCPconnectID, uint8_t sslCtxId)
{
    _TCPconnectID = TCPconnectID;

    sprintf(_buffer, "AT+QMTCFG=\"ssl\",%d,1,%d,0", _TCPconnectID, sslCtxId);
    wakeUp();
    if (!sendAndCheckReply(_buffer, _OK)) {
        if (_debug) Serial.println(F("[ERROR] Failed to configure QMTCFG ssl"));
        return false;
    }

    sprintf(_buffer, "AT+QMTCFG=\"version\",%d,1", _TCPconnectID);
    wakeUp();
    if (!sendAndCheckReply(_buffer, _OK)) {
        if (_debug) Serial.println(F("[ERROR] Failed to configure MQTT version"));
        return false;
    }
    ...
    wakeUp();
    if (!sendAndCheckReply(_buffer, _OK)) {
        if (_debug) Serial.println(F("[ERROR] QMTOPEN cmd"));
        return false;
    }
    ...
    while (millis() - t0 < 30000UL) {
        if (_uart->available()) {
            ...
            if (len < sizeof(_buffer) - 1) _buffer[len++] = c;
            if (c == '\n') {
                _buffer[len] = 0;
                len = 0;
                if (strstr(_buffer, "+QMTOPEN:")) {
                    char* p = strchr(_buffer, ',');
                    if (p) {
                        uint8_t stat = atoi(p + 1);
                        if (_debug) {
                            Serial.print(F("[MQTT] QMTOPEN stat="));
                            Serial.println(stat);
                        }
                        return stat == 0;
                    }
    ...
    if (_debug) Serial.println(F("[ERROR] QMTOPEN URC timeout"));
    return false;
}
...
bool QuectelBC660::connectMQTTSecure(const char* clientID,
    const char* username,
    const char* password)
{
    ...
    if (username && *username) {
        sprintf(_buffer, "AT+QMTCONN=%d,\"%s\",\"%s\",\"%s\"", _TCPconnectID,
            clientID, username, (password && *password) ? password : "");
    }
    else {
        sprintf(_buffer, "AT+QMTCONN=%d,\"%s\"", _TCPconnectID, clientID);
    }
