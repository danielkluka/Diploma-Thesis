...
class QuectelBC660 {
    public:
        ...
        bool manualRegisterToNetwork(uint32_t operatorCode, uint8_t mode = 4, uint8_t format = 2, uint32_t timeout = FIVE_MIN);
        ...
        bool configTLS(const char* caCert, const char* clientCert, const char* clientKey, uint8_t secLevel);
        
        bool openMQTTSecure(const char* host, uint16_t port, uint8_t TCPconnectID, uint8_t sslCtxId );
        bool openMQTT(const char* host, uint16_t port = 1883, uint8_t TCPconnectID = 0);
        bool closeMQTT();
        bool connectMQTTSecure(const char* clientID, const char* username, const char* password);
    ...
    private:
        // TLS/SSL support
        bool writePemTLS(const char* tag, const char* pem, uint32_t timeout);
